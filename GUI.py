...
class TrackPlot:
    ...
    def begin_measure(self, turnVal, speed):
        self.running = True

        # if we implement speed just comment out this line
        speed = 1
        self.hyp = math.sqrt(math.pow(self.x, 2) + math.pow(self.y, 2) + speed
                             - 2 * math.sqrt(math.pow(self.x, 2) + math.pow(self.y, 2))
                             * math.cos(math.pi * (1 - turnVal / 2)))
        if self.x != 0:
            self.theta1 = math.atan(self.y / self.x)
        else:
            self.theta1 = math.pi / 2
        self.theta2 = turnVal * math.pi / 2
        self.phi2 = math.pi - self.theta2
